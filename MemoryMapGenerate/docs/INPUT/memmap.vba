...
'            ElseIf tmpCalculation_Formula = "-----MEMORY(FUNCTION) Category-----" Then
...
'                Call DeleteDuplicateValue(tmpInfo)
'                tmpCalcList(cntArrayIndex) = tmpCalcList(cntArrayIndex) & vbCrLf & tmpInfo
            Else
                '配列に計算式を格納
                If tmpCalcBlockName = "" Then
                    tmpCalculation_Formula = Adjust_Indents(tmpCalculation_Formula, INDENTS_FORMULA2FULL)
...
'/* 概要   | 重複する要素を削除する
'/****************************************************************************************************
Sub DeleteDuplicateValue(tmpInfo As String)
    Dim dic As New Dictionary   ' 重複を除いた値を格納するDictionary
    Dim i As Integer            ' ループカウンタ
    Dim arEdit() As String      ' 編集後の配列
    ...
    For i = LBound(SectionList) To UBound(SectionList)
        ' 配列に未登録の値の場合
        If (dic.Exists(SectionList(i).Section_Category) = False) Then
            ' Dictionaryに追加
            Call dic.Add(SectionList(i).Section_Category, SectionList(i).Section_Category)
            
            ' 重複がない値のみを編集後配列に格納する
            arEdit(UBound(arEdit)) = SectionList(i).Section_Category
    ...
    ' 引数に編集後配列を文字列に変換して設定
    tmpInfo = ""
    For i = LBound(arEdit) To UBound(arEdit)
        If (arEdit(i) = "") Then
            arEdit(i) = "OTHER"
        End If
        If tmpInfo = "" Then
            arEdit(i) = Adjust_Indents(arEdit(i), INDENTS_FORMULA2FULL)
            tmpInfo = "/* " & arEdit(i) & " */"
        Else
            arEdit(i) = Adjust_Indents(arEdit(i), INDENTS_FORMULA2FULL)
            tmpInfo = tmpInfo & vbCrLf & "/* " & arEdit(i) & " */"
        End If
    Next
End Sub
